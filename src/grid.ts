...
export function fillGridWithDefaultValue(grid: Grid, val = 0): Grid {
  if (grid == null) throw new Error('grid is null or undefined');
  const clone: Grid = [];
  for (const row of grid) {
    const cloneRow = [];
    for (const cell of row) cloneRow.push(cell || val);
    clone.push(cloneRow);
  }
  return clone;
}

export function getBoxFromRolCol(row: number, col: number): number {
...
export function makeGrid(rowCnt: number, colCnt: number, val = 0): Grid {
